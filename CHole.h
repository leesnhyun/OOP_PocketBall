...
public:
	CHole();
	~CHole();
	bool create(IDirect3DDevice9* pDevice, D3DXCOLOR color = d3d::WHITE);	// 구멍을 화면에 생성함
	...
	void setCenter(float x, float y, float z);								// 구멍의 중심 좌표를 변경함
	float getRadius() const;												// 구멍의 반지름을 받아옴
	const D3DXMATRIX& getLocalTransform() const;							//
	void setLocalTransform(const D3DXMATRIX& mLocal);						//
	D3DXVECTOR3 getCenter(void) const;										// 구멍의 중심 좌표를 반환함
};
